...
use crate::{SemanticUncertaintyResult, RiskLevel, CalibrationMode, RequestId};
...
pub struct OSSLogitAdapter {
    ...
    fim_cache: std::collections::LRUCache<String, Array2<f64>>,
}
...
impl OSSLogitAdapter {
    ...
    pub fn new(framework: OSSModelFramework, config: AdapterConfig) -> Self {
        let fim_cache = std::collections::LRUCache::new(
            std::num::NonZeroUsize::new(config.fim_config.cache_size).unwrap()
        );
    ...
    pub fn analyze_logits(
        ...
        let (delta_mu, fim_precision) = if self.config.fim_config.enable_fim {
            let fim_result = self.calculate_fim_based_precision(&prob_distributions, logit_data)?;
            (fim_result.0, Some(fim_result.1))
        } else {
            (self.calculate_logit_precision(&prob_distributions)?, None)
        };
        
        // Calculate enhanced flexibility using paraphrases if available
        let (delta_sigma, max_js_divergence) = if let Some(ref paraphrase_logits) = logit_data.paraphrase_logits {
    ...
    fn calculate_fim_based_precision(&mut self, distributions: &[Array1<f64>], logit_data: &LogitData) -> Result<(f64, f64), Box<dyn std::error::Error>> {
        ...
        // Check cache first
        if let Some(cached_fim) = self.fim_cache.get(&cache_key) {
            let scalar_ic = self.compute_fim_scalar(cached_fim)?;
    ...
    fn calculate_enhanced_flexibility(&self, base_distributions: &[Array1<f64>], paraphrase_logits: &[Vec<Vec<f32>>]) -> Result<(f64, f64), Box<dyn std::error::Error>> {
        ...
        for paraphrase_logit_sequence in paraphrase_logits {
            ...
            for i in 0..min_len {
                let js_div = self.semantic_calculator.jensen_shannon_divergence(
                    &base_distributions[i], 
                    &paraphrase_distributions[i]
                )?;
        ...
        let enhanced_flexibility = max_js_divergence.max(base_flexibility);
        ...
        Ok((enhanced_flexibility, max_js_divergence))
    }
    ...
    fn calculate_geodesic_distances(&self, distributions: &[Array1<f64>]) -> Result<GeodesicDistances, Box<dyn std::error::Error>> {
        ...
        let max_distance = geodesic_distances.iter().fold(0.0, |a, &b| a.max(b));
    ...
    fn calculate_logit_flexibility(&self, distributions: &[Array1<f64>]) -> Result<f64, Box<dyn std::error::Error>> {
        ...
        for i in 0..distributions.len()-1 {
            ...
            let js_div = self.semantic_calculator.jensen_shannon_divergence(dist_a, dist_b)?;
    ...
    fn calculate_enhanced_logit_metrics(
        ...
        let confidence_score: f64 = distributions.iter()
            .map(|dist| dist.iter().fold(0.0, |a, &b| a.max(b)))
